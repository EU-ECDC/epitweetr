...
Migrating epitweetr from previous versions to version 2.0 or higher is possible without any data loss. On this section we will describe the necesary steps to performa the migration.

In Epitweetr v2 we have redesigned the way how tweets and series are stored. On previous versions tweets where saved as compressed json fies and series as RDS dataframes. On v2 we moved to a different storage sytem allowing efficient storage system allowing epitweetr to work as a search engine and at the same time allow efficient updates deletes and faster aggregation. In order to reach this objectives data is stored using Apache Lucene indexes.

If you have an existing installation that contains data on the previous format you have to perform a migration, following the steps detailed on this section. Any epitweetr version befor v2.0.0 will have to perform migration. You can also check if migration is needed any the folders tweets/geo or tweets/search contains json.gz files.

The migration steps are the following:
1.   Make sure you stop the data collection and alerts are stopped (you can see the status on the shiny configuration page. If you are using windows and you launch the tasks using the activate buttons you have to manually open the Windows task scheduler and disable the tasks
1.   Close the shiny App.
1.   Install the new epitweetr version (if you have not yet done it)
1.   Go to the configuration page on the shiny app
1.   Click on the 'Run dependencies' button
1.   Activate the 'Requirements and alerts' process. If you are on windows you can click on the activate button or you can run on a new R session ther following command: 
     -     epitweetr::detect_loop("data\_dir")
1.   The message under the dependencies tasks on the 'Requrements & alert pipeline' will start reporting the status and downloading the necessary dependencies from the selected maven mirror.
1.   If the embeddeed datanase is running, a message will appear asking to stop the database. You can do it by looking on open R sessions on your machne and killing the processes.
1.   After all dependencies have been downloaded a message will request to activate the embeded database, please refer to the section 'Setting up tweet collection and the alert detection loop' for details.
1.   Once the database is running the migration will start. Epitweetr will report the estimated remaining time to finish the migration. Be aware that depending on the size of your data this can take several days.
1.   As files are migrated to the new storage system, they are moved from the foldes tweets/search and tweets/geo to tweets/search\_archive and tweets/geo\_archive
1.   Once the migration is done you can backup and delete the \_archive folders.
1.   You can test that your data has been successfully migrated by searching for tweets on particular periods using the 'Data protection' page.
1.   Click on the button 'Run Geonames' and 'Run languages' wait for them to finish.
1.   You can now activateactivate the 'data collection & processing'.
...
-   500.000 tweets per month in the [essential access level](https://developer.twitter.com/en/docs/twitter-api/getting-started/about-twitter-api#v2-access-level)
...
For example, if the “search plan” begins at 4 am on the 10^th^ of November 2021, `epitweetr` will launch requests for tweets corresponding to its queries for the four-hour period from 4 am to midnight on the 10^th^ of November 2021. `epitweetr` starts by collecting the most recent tweets (the ones from 4 am) and continues backwards. If during the four-hour time period between 4am and midnight the API does not return any more results, the “search plan” for this query is considered completed.
...
In a parallel process to the collection of tweets, `epitweetr` attempts to geolocate all collected tweets using a supervised machine learning process. This process runs in automatically after tweets are collected.
...
Epitweetr counts five types of elements on tweets
...
Note that, differently to the other visualisation figures, the most frequent elements found in and extracted from tweets are always based on the geolocation related to “tweet location” and not to “user location” regardless of the filter selected in the dashboard. Topwords & hashtags and entities & context are shown in the same figure, respectively.

#### Aggregation of data

The aggregation process produces data on five subfolders of "fs": geolocated, country\_counts, topwords, urls, hashtags, entities and contexts. These folders are splitted on week subfolders and each contains a Lucene index with the aggregated information. This data can be extracted as a dataframe through the public package function 'get\_aggregates'
...
The user can choose to filter alerts based on the following filters: date, topics, countries/regions. These three filters will define the scope of alerts to search. Also two modifiers are available for searching:
...
In the manual tasks in the “Detection pipeline”, you have. You need to run manually the tasks of dependencies, geonames and languages by clicking in the buttons “Run dependencies”, “Run geonames” and “Run languages” the first time you use `epitweetr`, and then only if you are downloading new versions. Geonames and languages relate to the geolocation and language models used by epitweetr. If you would like to update them (this is not something that needs to be done regularly, more on a yearly basis or so), then you can click on “Run”. When running the 'download dependencies' tasks epitwteetr will request you to stop the embedded datanbase in order to perform the update. You can do this by looling on open R, Rsession or Rscript process on your system and killing them. 


The “Run alerts” button can be used to force the start of this tasks in case there is any error or issue. You can check their status in the “Detection Pipeline” table.
...
After activating the database, search and detection pipeline from the configuration page of `epitweetr` (Windows), three tasks will be created in the task scheduler and three terminal windows will be prompted. Please note that if the computer is logged/turned off or the terminal windows are closed, the search and detection pipeline will stop.
...
### Management of database, data collection and alerts loops (Linux and Mac)
