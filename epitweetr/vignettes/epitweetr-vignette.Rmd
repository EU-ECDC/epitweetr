...
<center>Fig 3: Shiny app data protection page</center>
...
In order to use `epitweetr`, you will need to collect tweets and run the alert detection loop (geonames, languages and alerts). Further details are also available in subsequent sections of the user documentation. A summary of the steps needed is as follows:
...
-   You will be able to visualise tweets after "Tweet search" and "dataase" are activated and the languages task has finished successfully.
...
#### Use of the Twitter Standard Search API version 1.1 and 2.0

`epitweetr` uses the Twitter Standard Search API version 1.1 and/or 2.0. The advantage of these API is that it is a free service provided by Twitter enabling users of `epitweetr` to access tweets free of charge. The search API is not meant to be an exhaustive source of tweets. It searches against a sample of recent tweets published in the past 7 days and it focuses on relevance and not completeness. This means that some tweets and users may be missing from search results.
...
-   **Using a Twitter developer app:** via `epitweetr` (app authentication)
    ...
    -   Follow the instuctions, answer the questions to activate to the Twitter API v2 using Essential access.
    ...
    -   Make a note of your OAuth settings
        ...
        -   You can activate V2.0 API on the config page
...
As a reminder, `epitweetr` is scheduled to make 180 requests (queries) to Twitter every 15 minutes with the user authentication (or 450 (v1.1) + 300 (v2) requests every 15 minutes if you are using Twitter developer app credentials depending on the API version you use). Each request can return 100 tweets. The requests return tweets and retweets. These are returned in JSON format, which is a light-weight data format.
...
This design can have the draw back of slowing down big topics collection since epitweetr is trying to rebuil last 7 days of history. If you do not care about rebuilding history on a particular point of time you can click into the "Dismiss past tweets" button who will discard all plans and will start collecting new data.

### Geolocation

In a parallel process to the collection of tweets, `epitweetr` attempts to geolocate all collected tweets using an supervised machine learning process. This process runs in automatically after tweets are collected.
...
To validate the candidate against geonames a score is associated with a greater probability that a match is correct. A score is:
...
On the Geotag page, epitweetr will allow you to download the data that was used to build the classifiers for location identificaion. The predefined annotations are based on location vs non location words. Locations are extracted from geonames.org data base. Non location words are obtained from common words on the downloaded models that are not present on geonames. You will be able to add tweets to the annotation datanbase and to manually annotate them until you reach the expected level of performance.

To help you on the evaluation process epitweetr will calculate standard machine learning metrics for evaluating its capacity to identify the right location in texts. These metrics are separately caculated by diferent type of texts: Tweet location, Tweet text, Tweet user description and total.
...
### Most frequent elements found in tweets

Epitweetr counts 5 types of elments on tweets

  - Words: Word presents on tweets. Words are then aggregated on the topwords time series

  - Hashtags: Tag present on tweets. Hashtags are then aggregated on the hashtags time series

  - Urls: Urls presents on tweets. Urls are then aggregated on the urls time series

  - Entities: Entities are automatically extracted and idetified by the twitter API v2. Thet are then aggregated on the entities time series. This information is only available when the V2 API is activated and properly configured. As per [*Twitter documentation*](https://developer.twitter.com/en/docs/labs/annotations) "Entities are comprised of people, places, products, and organizations... They are programmatically assigned based on what is explicitly mentioned in the Tweet text".  

  - Context: Context are automatically extracted and idetified by the twitter API v2. Thet are then aggregated on the contexts time series. This information is only available when the V2 API is activated and properly configured. As per [*Twitter documentation*](https://developer.twitter.com/en/docs/labs/annotations) "(Context are) Derived from the analysis of a Tweet’s text and will include a domain and entity pairing which can be used to discover Tweets on topics that may have been previously difficult to surface... a list of 50+ domains to categorize Tweets".  

Note that, differently to the other visualisation figures, the most frequent elemets in tweets are always based on the geolocation related to “tweet location” and not to “user location” regardless of the filter selected in the dashboard.

#### Aggregation of data

The aggregation process produces data on five sub folder of "fs": geolocated, country\_counts, topwords, urls, hashtags, entities and contexts. These folders are splitted on week subfolders and each contains a lucene index with the aggregated information. This data can be extracted as a dataframe through the public package funcion 'get\_aggregates'
...
The aggregation by top element is stored in the topwords, urls, hashtags, contexts and entities fs subfomders, they containss the number of tweets or retweets (or both) by topic, top element, date, country of tweet location and whether a tweet was a retweet or not (see screenshot).
...
-   The alert category estimated by epitweetr based on user annotations
...
The subscribers can receive real-time alerts (i.e. as soon as the detection loop is finalised) or scheduled alerts (e.g. once or twice a day). The subscribers list can be changed in the configuration page by downloading the Excel spreadsheet. This file has the following variables:
  ...
  - “One tweet alerts”: Whether the user subscrive to alerts including just one tweet
...
-   "fs", which stores the lucene indexes for storing tweets and aggregated time series

-   "geo", which stores the GeoNames data as text, index files, tweets waiting to be geolocated and settings about the geolocation algorithm.

-   "alert-ml", which stores the machine learning models used for alert classification
...
-   "collections" stores the aggregations that will be performed by epitweetr 


#### Fs folder &gt; tweets
In the fs folder, the subfolder “tweets” has contains the lucene indexes storing the content of the collected tweets and the geolocation information.
...
In the *series* folders, `epitweetr` stores the aggregated data of the geolocated tweets as well as the top elements identified.

Each folder is named matchning the respective time series and smitted by ISO week of date of publication. Each weekly folder contains a Lucene Index.

```{r echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("https://github.com/EU-ECDC/epitweetr/raw/master/img/fs_folder.png")
```
...
-   The alerts page, where a user can view the current alerts and train machine learning algorithms for alert classification on user defined categories

-   The geotag evaluation page, where a user can evaluate the geolocation algorithm and provide annotations for improve its performance

-   The data protection page, where a user can search, anonymise and delete tweets from the epitweetr database to honor data deletion requests from third parties
...
The dashboard is where you can interactively explore visualisations of tweets. It includes a line graph (trend line) with alerts, a map and top words, urls, hashtags, entiteis and contexts of tweets for a given topic. After selecting the expected parameters you have to press the 'Run' button in order to see or refresh the report.
...
#### Most frequent words, hashtags, urls, context, entities found in tweets

```{r echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("https://github.com/EU-ECDC/epitweetr/raw/master/img/top_words.png")
```

This graph then displays the top elements of tweets by topic for the study period for the geographical units chosen, and according to the filter on tweet/retweet.
...
In the case of urls the resuls are displayed as a tabme with the ability of clicking directly to the most important links

### The alerts page

The alerts page summarises the signals detected within the specified study period and provide functionality for adding annotations for training machine learning algorithhms to classify alerts on user defined topics. This screen is splitte on two sections

#### Find alerts

The user can choose to filter alerts based on the following filters: Date, topics, countries/regions. These three filters will define the scope of alerts to search. Also two modifiers are available for search
...
-   Display: Choose the columns to display on the search results
    ...
    -    Parameters: Focused on showing the parameters of the alerts that were produced. The following columns are displayed for alerts: Date, Hour, Topic Region, Category, Tops, Tweets, % important user, Threshold Baseline, Bonf. corr., Same weekday baseline Day, rank,  With retweets Location,  Alert FPR (alpha), Outlier FPR (alpha). 
...
-   Add alerts to annotation: The alerts returned by the search are added to the annotations database so the user can classify them.

#### Alerts annotations

Epitwweetr classify alerts using topwords and toptweets using annotations provided ny end users using the alert annotation spreadsheet. The user can define different algorihms and its parameters and epitweetr will find the best algorithm to perform the classification. Epitweetr will random split of annotation letting 25% of alerts for evaluation and 75% for training. All the user requested algorithms will be tested and the best in terms of f1Score will be selected to classify new alerts. A final train is done with the best algorithm using all annotations.
...
-   Runs sheet: A registry of algorithms to test including performance metrics on the last evaluation.
    -    Ranking: The ranking of the algorithm in terms of f1score
    ...
    -    Runs: The number of runs to perform to evaluate the algorithm. Each run will make a different random split.
    -    F1Score: The F1score of all classes as provided by Apache Spark [*MulticlassClassificationEvaluator*](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/ml/evaluation/MulticlassClassificationEvaluator.html)
    ...
    -    Precision by Class: The Precision by Class of all classes as provided by Apache Spark [*MulticlassClassificationEvaluator*](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/ml/evaluation/MulticlassClassificationEvaluator.html)
    -    Sensitivity by Class: The Sensitivity by Class of all classes as provided by Apache Spark [*MulticlassClassificationEvaluator*](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/ml/evaluation/MulticlassClassificationEvaluator.html)
    -    FScore by Class: The FScore by class of all classes as provided by Apache Spark [*MulticlassClassificationEvaluator*](https://spark.apache.org/docs/latest/api/scala/org/apache/spark/ml/evaluation/MulticlassClassificationEvaluator.html)
...
-   Geolocate annotations: Pressing this button will add the selected number of annotations to the epitweetr geolocation spreaddsheet and evaluate the geoloation algorithm on the current annotations
...
-   Type: This column is a reference for the user to understun the kind of annotation. It can be any of the following.
    -   Text: A text that can contains locations inside it can be a tweet or a user description.
    -   Location: A text containint a location.
    -   Person: A known person e.g. presidents that can be used by epitweetr to detect countries. This data is not used for training, just for ovewriting the algorithm behavior.
    -   Demonym: A known word to associate to a country e.g. chilean. This data is not used for training, just for ovewriting the algorithm behavior.
...
-   Source: The source of the annotation, it can be any of: Epitweeter database (provided by epitweetr team), Epitweetr model (obtained automatically by epitweetr) or Tweet (obtained from a tweet).
-   Tweet Id: The tweet id if the text is associated to a tweet.
-   Lang: The language associated to the annotation.
-   Tweet part: The tweet part if associated to a tweet. It can be any of: "text", "user desctiption" or "user location".
-   Epitweetr match: The location found by epitweetr.
-   Epitweetr country match: The country of the location found by epitweer.
...
knitr::include_graphics("https://github.com/EU-ECDC/epitweetr/raw/master/img/geotag_evaluation.png")
```
### The data protcetion page
In the data protection page, the user can search, anonymise and delete tweets from the epitweetr database to honor data deletion requests from third parties.

The following search filters are available: Topic, period, country & regions, mentioning (for selecting only mentions of the provided users), from users (for selecting only tweets from the provided users) and both (for getting tweets either metnioning of from the provided user)

The folliwing controls ara available:
-   Limit: Limit the perimeter of the search to the first "limit" tweets
-   Search: Performq the search and show the results on the screen.
-   Anonym search: Perforp the search and show anonymized results on the screen. User mentions and authors are replaced by a mention USER.
-   Anonymize: Permanantly replace all matching tweets user mentions and authors  by USER.
-   Delete: Permanently deletea all the matching tweets.
...
In the status section, you can tell if the embedded database, the search pipeline and the detect pipeline processes are running. If you are running Windows, you can click on “activate” to register these processes as scheduled tasks and run them manually from the Windows task scheduler.
...
Alerts dates should be more recent if the search and detection pipelines are active and running. They are scheduled according to the detect span. The status can include running, scheduled, pending, failed or aborted (if it has failed more than three times).
...
-   **Stream database results:** Wether to store files on disk instead of peforming streaming evaluation of aggregated dataframes coming from the datanase..
...
The admin email will receuve epitweetr notifications of regular sanity checks of the application indicating by example if some of the loop has failed.
