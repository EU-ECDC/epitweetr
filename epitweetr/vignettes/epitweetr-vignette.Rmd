...
If previous signals are included without modification in the baseline to calculate the mean and standard deviation for the signal detection, then the mean and standard deviation will be larger, which may mean important current signals will not be detected. To address this, epitweetr downweights previous signals, so that they are taken into account in the signal detection process and thus the mean and standard deviation are lower. Data points that are not signals are given a weight of "1" and signals are given a weight lower than one, which is lower the greater the difference between the signal and the threshold is <span style="color:red">It is not 100% clear to me if the threshold here is the same as the threshold as calculated by the baseline, can you confirm?</span>. This is an approach used in the Farrington algorithm <span style="color:red">Link to Farrington reference </span>. Details on the downweighting can be found in Appendix 1 of this user documentation.


#### Timing of signal detection

Signal detection is carried out based on “days”<span style="color:red"> (Question: if we choose "weeks" as a time unit, will the baseline be calculated on weeks? I need to test)</span>,<span style="color:blue">Answer: NO, when week is selected we perform all calculations as for daily and then we group. Showing alerts if a daily alert happened on the week </span> which are moving windows of 24 hours, moving according to the detect span <span style="color:red">Is it search or detect span??</span><span style="color:blue">No there is no direct relationship with the search or detect span. The 24 hours block starts the hour before the last aggregated tweet</span> (see also section *The interactive user application (Shiny app) > The configuration tab > General*). The baseline is calculated on these “days” from -1 to -8 (if the current "day" is zero).
...
<span style="color:red">New section, please review. </span>
<span style="color:blue">reviewed with small fixes. </span>
It is possible that there is a "day of the week effect", where more tweets may be tweeted on a given day of the week (e.g. Monday) than on other days. To avoid this, you can also select to calculate the baseline not on consecutive days, but on the past N days that correspond to the same 24 hour window N days back. This way if N = 7, the baseline is calculated using the “days” from -7, -14, -21, -28, -35, -42, -54 and -56 (if the current "day" is zero).

This option is on the configuration page of the Shiny app "Default same weekday baseline".

#### Sending email alerts

Emails containing a list of signals detected are sent automatically by epitweetr according to the detect span and the subscribers list. Due to the time necessary to collect, geolocate and aggregate the tweets, email alerts will miss the most recent tweets that has not yet gone to these process. The lag between tweets and alerts is excpected to be less than (2 * ( collect\_span ) + detect\_span) which should be 3h30 usiing defaul values. <span style="color:red">Do we have details on this at all, Francisco?</span>.
<span style="color:blue">rewritten </span>
...
-   The percentage of tweets from trusted users
...
This information is also available in the "Alerts" tab of the Shiny app. <span style="color:red">I cannot see this tab yet, so I cannot add anything about it here</span><span style="color:blue">Which information do ypu need?</span>

###  Folder structure

<span style="color:red">Updated section, please review</span>
<span style="color:blue">completed</span>

epitweetr stores tweets, aggregations and the configuration in the “data folder” you have to choose when launching the application.

Within the **data** folder there are 3 JSON files, 
-   properties.json, generated from the information on the Geberal properties of the Shiny app

-   topics.json managed by the search loop it keeps a track of tweet collection plan and progress

-   tasks.json managed by the detect loop it keeps information and status of the different tasks done by this process.

also the following sufolders

-   "geo", which stores the geonames data as a text and index files <span style="color:red">Francisco, is this  true?</span><span style="color:blue">fixed</span>

-   "hadoop",which stores Spark dependencies for Windows operating systems <span style="color:red">Francisco, is this  true?</span><span style="color:blue">Yes it is!!</span>

-   "jars", where collections of Java dependencies are stored that are needed in the geolocation and aggregate process <span style="color:red">Francisco, is this  true?</span><span style="color:blue">Almost I have fixed it</span>

-   "languages", where the fasttext files indexes and models are stored that are used to perform geolocation in the tweet text

-   "stats", storing json files reporting statistics used to optimise the aggregate process by linking tweets files and posted dates of tweets <span style="color:red">Francisco, what does this do?</span><span style="color:blue">filled</span>, and

-   "alerts" storing json files of alerts detected by the detect loop.

-   "series" and "tweets" that are explained in more detail below.
...
-   geolocated_YYY.MM.DD.Rds contains daily tweet counts at finest possible location level

-   topwords_YYY.MM.DD.Rds contains daily tweet top words counts at country level

-   country\_counts_YYY.MM.DD.Rds contains hourly tweet counts at country level
...
-   The alert tab, where you can view the current alerts and associated information

-   The "geotag evaluation" tab, where you can evaluate the geolocation algorithm in different tweet fields to manually choose the geolocation threshold <span style="color:red">What does this do?</span> <span style="color:blue">Filled</span>
...
It is possible that there is a "day of the week effect", where more tweets may be tweeted on a given day of the week (e.g. Monday) than on other days. You can also select to calculate the baseline not on consecutive days, but on the past 7 days that correspond to the same 24 hour window 7 days back. This way the baseline is calculated using the “days” from -8 to -56 (if the current "day" is zero).
...
-   Using a Twitter account: delegated via rtweet (user authentication)
    ...
    -   A pop-up window will appear <span style="color:red"> Francisco is this still applicable? I don't remember a pop-up window. I would also like to take a screenshot, but I am not sure if there is a problem on the config page. Should some of the text change if you click on Twitter account vs. Twitter developer app? </span></span><span style="color:blue">This is correct, if you want to see the page you just have to click on update properties</span>, where you enter your Twitter user name and password and then the created token will validate the fact that the application can access Twitter on your behalf. You will send this token each time you access tweets.
...
<span style="color:red">I don't think we have time to include this, what do you think, Francisco?</span>
