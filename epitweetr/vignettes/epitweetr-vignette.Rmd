...
epitweetr is conceived to be platform independent,  working both on Windows and Linux (Mac is untested) <p style="color:red">Francisco, do we keep this in?</span>. We recommend that you use epitweetr on a computer that can be run continuously. You can switch the computer off, but you may miss some tweets if the downtime is large enough, which will have implications for the alert detection. Before using epitweetr the following items need to be installed:
...
<p style="color:red">**CHECK with Francisco if this is still valid:**</p><p style="color:blue">**Yes it is:**</p>
...
epitweetr_app("data_dir")
```
-   On the configuration page of the Shiny app, in the manual tasks of the "Detection pipeline", click on "Run dependencies" <p style="color:red">Francsico, is this only for Windows?</span>, "Run geonames" and "Run languages" (their status will change to “pending”). This downloads the elements needed for the detection pipeline.
...
The threshold is empirically chosen and can be evaluated against a human read of tweets and tweet locations, on the geolo tag evaluation tab.
...
The geolocation of the match is stored as a country code (using the ISO 3166 standard) and a longitude and latitude associated with the exact geolocation. This is stored in the aggregated data (see section <span style="color:red">x</span>).
...
<span style="color:red">Esther update screenshot and review this section when epitweetr working </span>
During the geolocation process, the country\_counts Rds file is also created, which is used to create the trendline in the Shiny app. This is a smaller Rds file, without the longitude and latitude information, and includes the number of tweets by hour within a day, by country (according to tweet location or user location), topic (see screenshot), and whether a tweet was a retweet or not. The known\_retweets and known\_original fields give the number of tweets or retweets from a list of “specific users”. See section <span style="color:red">x</span> for more information on “specific users”.
...
This information is also available in the "Alerts" tab of the Shiny app. <span style="color:red">I cannot see this tab yet, so I cannot add anything about it here</span>
...
#### Data folder &gt; tweets
<span style="color:red">Francisco, this doesn't work on my machine, so can you review?</span>
...
-   The "geotag evaluation" tab, where you can evaluate the geolocation algorithm in different tweet fields to manually choode the geolocation threshold <span style="color:red">What does this do?</span> <span style="color:blue">Filled</span>
...
<span style="color:red">I haven't reviewed this yet, I will review it in the morning </span>
