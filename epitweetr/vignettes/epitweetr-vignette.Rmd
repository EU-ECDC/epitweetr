...
epitweetr is conceived to be an independent platform that works both on Windows and Linux (Mac is untested) <p style="color:red">Francisco, do we keep this in?</span>. We recommend that you use epitweetr on a computer that can be run continuously. You can switch the computer off, but you may miss some tweets, which will have implications for the alert detection. Before using epitweetr the following items need to be installed:

### Prerequisites for running epitweetr

-   R version 3.6.3 or higher

-   Java 1.8 eg. openjdk version "1.8" [*https://www.java.com/download/*]. The 32-bit rather than the 64-bit version is preferred, due to potential memory constraints. <p style="color:red">Francisco please check this</span>
...
If the command returns Null, then you will need to set the Java Home environment variable, using the Sys.setenv() function.

<p style="color:red">**CHECK with Francisco if this is still valid:**</p>
...
You have two options for authentication to collect tweets, using a **Twitter account** (this approach utilises the rtweet package) and using a **Twitter application**. For the latter, you will need a Twitter developer account, which can take some time to obtain, due to verification procedures. We recommend for testing purposes and short-term use the first option (using a Twitter account via the rtweet package), but for long-term use the Twitter developer app.
...
In a parallel process to the collection of tweets, epitweetr attempts to geocode all collected tweets using a machine learning process. This process runs in the same schedule window as the collection of tweets (e.g. if a 4-hour window is set for tweet collection this will also count for the geocoding, with as many as possible of the newest tweets being geocoded first followed by any non-geocoded older tweets from previous 4-hour windows).
...
Some tweet text will give rise to several locations through the matching process, however epitweetr only chooses one location per tweet. In order to choose the most relevant location, a score is calculated for each match. The algorithm also adds further words to the set and if the score grows by using more words, the algorithm tries to find a local maximum of the score with a bigger text.
...
Emails containing a list of signals detected are sent automatically by epitweetr according to the detect span. Signals very recently encountered and previously reported are not included in the current email alert <span style="color:red">Do we have details on this at all, Francisco?</span>.
...
Within the **data** folder there are 3 JSON files, the properties.json, the topics.json and the tasks.json file. They are generated from the information on the Shiny app configuration tab. There are also the subfolders

-   "geo", which stores the geonames data as a text file <span style="color:red">Francisco, is this  true?</span>

-   "hadoop",which stores Spark dependencies for Windows operating systems <span style="color:red">Francisco, is this  true?</span>

-   "jars", where collections of Java files are stored that are needed in the geolocation process <span style="color:red">Francisco, is this  true?</span>

-   "languages", where the fasttext files are stored that are used to perform geolocation in the tweet text

-   "stats", <span style="color:red">Francisco, what does this do?</span>, and
...
-   The "geotag evaluation" tab, where you can evaluate... <span style="color:red">What does this do?</span>
