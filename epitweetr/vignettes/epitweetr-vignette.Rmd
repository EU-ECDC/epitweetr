...
-   If you are running it in Windows, you will also need Microsoft Visual C++, however in most cases it is likely to be pre-installed:
    ...
    -   Microsoft Visual C++ 2010 Redistributable Package (x64) [*https://www.microsoft.com/en-us/download/details.aspx?id=40784*]
...
-   Make sure the 'data collection & processing' and 'requirements & alerts' pipelines are stopped (you can see the status on the shiny configuration page). If you are using Windows and you have launched the tasks using the activate buttons, you have to manually open the Windows task scheduler and disable these two epitweetr tasks.
-   Close the Shiny App.
...
-   Activate the 'Requirements and alerts' process. If you are on Windows, you can click on the activate button or you can run on a new R session executing the following command: 
     -     epitweetr::detect_loop("data\_dir")
-   In the line of the dependencies tasks under 'Requirements & alerts' table, there will be a message reporting the status of the migration and download of necessary dependencies from the selected maven mirror. It will indicate whether some of the epitweetr tasks must be stopped or activate it to complete migration and download of dependiencies.
-   First, it will download the dependencies for which the embedded epitweetr database should be stop. If the embedded database is running, a message will appear asking you to stop the database ('epitweetr database' pipeline).
...
-   Once the epitweetr database is running, the migration will start. Epitweetr will report the estimated remaining time to finish the migration. Be aware that depending on the size of your data, this can take several days for which the machine running epitweetr cannot be turned off.
...
### Management of 'epitweetr database' and 'Data collection and alerts' pipelines (Windows)
...
### Management of 'epitweetr database' and 'Data collection and alerts' pipelines (Linux and Mac)
...
### Running 'epitweetr database', 'Data collection and alerts' and 'Requirements and alerts' pipelines
