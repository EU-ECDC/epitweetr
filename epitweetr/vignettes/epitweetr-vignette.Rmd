...
In Epitweetr v2 we have redesigned the way how tweets and series are stored. On previous versions tweets where saved as compressed json fies and series as RDS dataframes on 'tweets' and 'series' folder respectively. On v2 we moved to a different storage sytem allowing epitweetr to work as a search engine and at the same time allow efficient updates deletes and faster aggregation. In order to reach this objectives data is stored using Apache Lucene indexes on the 'fs' folder. Note that during migration tweets data is moved to the 'fs' folder series is left as it is. Epitweetr reports will combine data from older and new storage system.

If you have an existing installation that contains data on the previous format you have to perform a migration, following the steps detailed on this section. This would be the case for any epitweetr version befor v2.0.0. You can also check this by looking on 'tweets/geo' or 'tweets/search' folders. If there is any json.gz files migration is needed.

The migration steps are the following:

-   Make sure the data collection and alerts tasks are stopped (you can see the status on the shiny configuration page). If you are using windows and you have launched the tasks using the activate buttons you have to manually open the Windows task scheduler and disable the epitweetr tasks.
-   Close the shiny App.
-   Install the new epitweetr version (if you have not yet done it).
-   Go to the configuration page on the shiny app.
-   Click on the 'Run dependencies' button.
-   Activate the 'Requirements and alerts' process. If you are on windows you can click on the activate button or you can run on a new R session ther following command: 
     -     epitweetr::detect_loop("data\_dir")
-   The message under the dependencies tasks on the 'Requrements & alert pipeline' table will start reporting the status and downloading the necessary dependencies from the selected maven mirror.
-   If the embeddeed datanase is running, a message will appear asking to stop the database. You can do it by looking on open R sessions on your machne and killing the processes.
-   After all dependencies have been downloaded a message will request to activate the embeded database, please refer to the section 'Setting up tweet collection and the alert detection loop' for details.
-   Once the database is running the migration will start. Epitweetr will report the estimated remaining time to finish the migration. Be aware that depending on the size of your data this can take several days.
-   As files are migrated to the new storage system, they are moved from the foldes 'tweets/search' and 'tweets/geo' to 'tweets/search\_archive' and 'tweets/geo\_archive'.
-   Once the migration is done you can backup and delete the \_archive folders.
-   You can test that your data has been successfully migrated by searching for tweets on particular periods using the 'Data protection' page.
-   Click on the button 'Run Geonames' and 'Run languages' wait for them to finish.
...
### Management of satabase, data collection and alert loops (Windows)
