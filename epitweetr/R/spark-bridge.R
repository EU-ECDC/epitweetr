...
#' Install spark & lucene dependencies, which consists on the following tasks
#'  - Download spark and lucene dependencies from configuration maven repo to project data folder. Java packages to be downloaded are defined on package file 'sbt-deps.txt'
...
#' @export
download_spark_dependencies <- function(tasks = get_tasks()) {
  tasks <- tryCatch({
    tasks <- update_spark_task(tasks, "running", "getting sbt dependencies", start = TRUE)
    tasks <- download_sbt_dependencies(tasks)
    tasks <- update_spark_task(tasks, "running", "sending emails")
    if(.Platform$OS.type == "windows") {
      tasks <- download_winutils()
    }
    # Setting status to succes
    tasks <- update_spark_task(tasks, "success", "", end = TRUE)
    tasks
  }, error = function(error_condition) {
    # Setting status to failed
    message("Failed...")
    message(paste("failed while", tasks$spark$message," :", error_condition))
    tasks <- update_spark_task(tasks, "failed", paste("failed while", tasks$spark$message," getting spark dependencies ", error_condition))
    tasks
...
#' Download SBT dependencies from maven (spark, lucene, netlib (BLAS), httpclient) into package folder
download_sbt_dependencies <- function(tasks = get_tasks()) {
  if(!exists("maven_repo", where = tasks$spark)) stop("Before running detect loop you have to manually update 'Java/Scala dependencies' to set the used repository")
  ...
  urls <- paste(tasks$spark$maven_repo , gsub("\\.", "/", sapply(parts, `[[`, 3)), sapply(parts, `[[`, 4), versions, sapply(parts, `[[`, 5), sep = "/")
  urls2 <- paste(tasks$spark$conf$maven_repo , gsub("\\.", "/", sapply(parts, `[[`, 3)), sapply(parts, `[[`, 4), versions2, sapply(parts, `[[`, 5), sep = "/")
  ...
  for(i in 1:length(urls)) {
    tryCatch({
      tasks <- update_spark_task(tasks, "running", paste("downloading", urls[[i]]))
      download.file(url = urls[[i]], destfile = dest_file[[i]])
    }
    ,warning = function(c) {
      tasks <- update_spark_task(tasks, "running", paste("downloading", urls2[[i]]))
...
download_winutils <- function(tasks = get_tasks()) {
  ...
  if(!exists("winutils", where = tasks$spark)) stop("Before running detect loop you have to manually activate 'Java/Scala dependencies' to set the winutils to use")
  tasks <- update_spark_task(tasks, "running", paste("downloading", conf$winutils_url))
  download.file(url = tasks$spark$winutils_url, destfile = file.path(get_hadoop_home_path(), "bin"))
  retutrn(tasks)
} 
