...
trait GeonamesTest extends UnitTest {
   ...
   it should "find places with vectors" in {
     ...
     import spark.implicits._
     val geonames = conf.geonames
     val vectorText =  geonames.geolocateText(text=Seq("Viva Chile"), lang = Seq("es"), reuseGeoIndex = true, maxLevDistance = 0, minScore = 0, nBefore = 4, nAfter = 4, langs = conf.languages.get, langIndexPath = conf.langIndexPath)
     vectorText.show
